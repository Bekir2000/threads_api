...
app.get('/callback', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            req.session.access_token = longLivedToken;
            console.log('Successfully exchanged for long-lived token');
        } catch (exchangeError) {
            console.warn('Failed to exchange for long-lived token, using short-lived token:', exchangeError.message);
            req.session.access_token = shortLivedToken;
        }
...
app.get('/account', loggedInUserChecker, async (req, res) => {
    ...
    console.log('user details url: ', getUserDetailsUrl)
    console.log('long living token used: ', req.session.access_token);
...
app.get('/userInsights', loggedInUserChecker, async (req, res) => {
    ...
    console.log('user insights url: ', queryThreadUrl)
 ...
 * @param {string} shortLivedToken The short-lived access token to exchange
 * @returns {Promise<string>} The long-lived access token
 */
async function exchangeToken(shortLivedToken) {
    const exchangeUrl = buildGraphAPIURL('access_token', {
        grant_type: 'th_exchange_token',
        client_secret: API_SECRET,
    }, shortLivedToken);

    try {
        const response = await axios.get(exchangeUrl, { httpsAgent: agent });
        return response.data.access_token;
    } catch (error) {
        console.error('Error exchanging token:', error?.response?.data?.error?.message ?? error.message);
        throw error;
    }
}
/**
